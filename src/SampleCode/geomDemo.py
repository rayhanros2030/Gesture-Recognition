...
def scaleDemo(img1, img2):
    ...
    for scale in [0.25, 0.5, 1.75, 2]:
        print("fx =", scale)
        newImg = cv2.resize(img1, (0, 0), fx=scale, fy=1.0)
        cv2.imshow("Scaled", newImg)
        cv2.waitKey()
...
def translateDemo(img):
    ...
    for (xDist, yDist) in [(50, 50), (50, 200), (-10, 50), (-50, -50)]:
        ...
        newIm = cv2.warpAffine(img, transMatrix, (wid, hgt))
...
cv2.imshow("Beach original", beach)
cv2.imshow("Mountain original", mountain)
